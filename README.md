...
- **Lampiran** — layar ini mengambil aturan validasi lampiran dari server saat dibuka (`GET /api/v1/documents/config?referenceType=hr_leave_requests`), lalu menyesuaikan UI otomatis: label "Lampiran (wajib)" atau "Lampiran (opsional)", single atau multi-file picker (tombol "+ Tambah File" sampai batas jumlah file tercapai), dan batas ukuran/ekstensi file — semuanya bisa diubah dari ERP Web (*Document → Document Settings*) tanpa update aplikasi mobile. Default saat ini: maksimal 3 file, PDF/JPG/JPEG/PNG/DOCX, 5 MB per file, tidak wajib. File dipilih pakai package `file_picker`.
- Alasan opsional.
