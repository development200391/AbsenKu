...
- **Lampiran** — layar ini mengambil aturan validasi dari server saat dibuka (`GET /api/v1/documents/config?referenceType=hr_leave_requests`), yang isinya sekarang berupa DAFTAR SLOT (bukan satu aturan gabungan) — tiap slot punya nama, wajib/opsional, batas ukuran, dan ekstensinya sendiri-sendiri, sesuai yang diatur di ERP Web (*Document → Document Settings*) tanpa perlu update aplikasi mobile. Layar merender satu baris per slot: nama slot + tag "Wajib"/"Opsional", file picker single-file miliknya sendiri, dan catatan (note) miliknya sendiri — bukan satu catatan gabunganC untuk semua lampiran. Default saat ini: 3 slot bernama "Leave Request 1/2/3", PDF/JPG/JPEG/PNG/DOCX, 5 MB per file, tidak wajib. File dipilih pakai package `file_picker`.
- Alasan opsional.
- Kirim → leave request beserta semua lampirannya dikirim dalam **satu request** sekaligus (bukan dua langkah terpisah lagi), lalu status jadi **Menunggu (Pending)**, belum resmi sampai disetujui HR/atasan lewat ERP Web (*HR → Leave → Requests*). Kalau ditolak melebihi kuota jenis cuti tersebut untuk tahun berjalan, submit akan gagal dengan pesan error dari server tanpa ada yang tersimpan. Kalau leave request-nya sendiri berhasil tersimpan tapi salah satu file gagal diupload (misal corrupt), submit tetap dianggap sukses dan muncul dialog peringatan berisi file mana yang gagal.
...
- **Buka lampiran** — tap salah satu file di daftar lampiran men-download file-nya (`GET /api/v1/documents/{id}/download`, lewat `LeaveRepository.downloadAttachment`), simpan sementara ke temp folder HP, lalu buka pakai app default OS untuk tipe file itu (PDF viewer, Galeri, WPS, dst — lewat package `open_filex`). Ada indikator loading di baris file yang lagi didownload, dan pesan error di baris itu juga kalau gagal (mis. tidak ada app yang bisa buka tipe file itu). Widget-nya (`AttachmentListTile`) dipakai bareng oleh layar ini DAN Approval Inbox (poin 5), supaya perilakunya konsisten di dua tempat.
- Endpoint: `GET/POST /api/v1/hr/leave-requests/self` (submit gabungan field + lampiran dalam satu multipart request), `GET /api/v1/hr/leave-requests/self/leave-types`, `GET /api/v1/documents/config?referenceType=` (aturan validasi lampiran), `GET /api/v1/documents` (list lampiran untuk Riwayat Cuti), `GET /api/v1/documents/{id}/download` (isi file, dipakai fitur "Buka lampiran" di atas).
...
- **Lihat detail sebelum approve/reject** — tap baris (bukan tombol ✓/✕-nya) membuka bottom sheet berisi detail lengkap: nama karyawan, jenis cuti, rentang tanggal, dan **alasan** (field yang tidak muncul di daftar utama, karena `Subject`-nya generik lintas modul). Ada juga tombol lampiran (📎) yang buka daftar file yang diupload si pengaju (mis. surat dokter) — sama seperti Riwayat Cuti (poin 4), file-nya bisa langsung di-tap untuk didownload & dibuka pakai app default OS (`AttachmentListTile`, komponen yang sama dipakai kedua layar), cuma di sini sumber datanya punya ORANG LAIN, bukan diri sendiri. Tombol Approve/Reject juga ada di dalam sheet ini, jadi bisa langsung bertindak tanpa tutup sheet dulu. **Detail ini cuma muncul untuk `referenceType` yang dikenal app** (baru `hr_leave_requests`) — request dari modul lain yang belum didukung tetap tampil di daftar tapi barisnya tidak bisa di-tap, tidak ada dead-end.
- Endpoint: `GET /api/v1/approval/dashboard` (jumlah untuk badge di Beranda), `GET /api/v1/approval/inbox` (daftar), `POST /api/v1/approval/requests/{id}/actions/approve`, `POST /api/v1/approval/requests/{id}/actions/reject`, `GET /api/v1/hr/leave-requests/{id}` (detail cuti **milik siapapun**, dipakai khusus untuk bottom sheet ini — beda dari endpoint `/self` yang dipakai poin 4, endpoint ini tidak dibatasi kepemilikan sama sekali di sisi server), `GET /api/v1/documents?referenceType=hr_leave_requests&referenceId=` + `GET /api/v1/documents/{id}/download` (lampiran, lewat `LeaveRepository` yang sama dipakai poin 4).

**Catatan penting (fix di backend)**: sebelum lampiran cuti bisa dilihat dari sini, `DocumentService.EnsureLeaveRequestAccessAsync` di `ERP.API` cuma mengizinkan pemilik record, user tanpa profil karyawan, atau role Super Admin/HR Manager/HR Staff — seorang manajer departemen biasa (approver via DirectSuperior) BUKAN salah satu dari itu, jadi bakal ditolak (403) walau dia justru orang yang paling perlu lihat buktinya sebelum approve. Sudah ditambal dengan kondisi baru: siapapun yang punya *approval step aktif* untuk leave request itu juga diizinkan. Detail di `D:\NET\SINARA\ReadMeGeneralApproval.md`.
...
lib/
  ...
  features/
    ...
    leave/
      ...
      data/        # pemanggilan API leave-requests self-service + getById (non-self,
                   # dipakai layar Approval Inbox untuk lihat cuti milik orang lain) +
                   # downloadAttachment
      presentation/# layar Ajukan Cuti/Sakit, Riwayat Cuti, AttachmentListTile (dipakai
                   # bareng oleh Riwayat Cuti & Approval Inbox)
    approval/
      ...
      presentation/# layar Approval Inbox (bergantung juga ke LeaveRepository utk detail cuti)
  l10n/            # file ARB (id/en/ja) + AppLocalizations hasil generate (flutter gen-l10n)
```
File inti di `core/`: `api_client.dart`, `api_exception.dart` (tipe error bersama: `ConnectionException`/`ApiException`/`UnknownApiException`), `auth_session.dart`, `biometric_auth_service.dart`, `locale_controller.dart`, `language_picker.dart`, `secure_storage.dart`, `file_opener.dart` (download bytes → temp file → buka pakai app OS default, dipakai `AttachmentListTile`).
...
- **Buka lampiran** (poin 4 & 5) pakai package `open_filex`, yang otomatis mendaftarkan `FileProvider`-nya sendiri (authority `${applicationId}.fileProvider`) — biasanya tidak perlu config manual di `AndroidManifest.xml`. **Kalau muncul error build soal konflik `FileProvider`/manifest merge** (biasanya kalau ada plugin lain yang juga daftar `FileProvider`), baru perlu tambah provider manual + `android/app/src/main/res/xml/filepaths.xml` — lihat dokumentasi `open_filex` di pub.dev untuk XML-nya persis.
